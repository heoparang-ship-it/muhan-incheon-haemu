...
1. 걷기 4×4 시트 (지금은 4방향 대기 + 걷기 바운스). 별도 PR `cursor/commando-walk-art-f6c7` 가 열려 있음.
2. 담장(`kind==='wall'`)은 아직 코드 드로우. 별도 PR `cursor/wall-prerender-9678` 가 열려 있음.
3. 백서 3차: D14 리플레이 고스트, D15 접근성 (다른 PR)
4. 실제 샘플 효과음 없음 (신스만)

D13 2장 프레임은 2026-08-22 에 넣음. 1장 배치표를 `CHAPTERS.gamnaru`로 분리. 2장 맵은 없음. 맵 지문·조수 공식은 그대로.
