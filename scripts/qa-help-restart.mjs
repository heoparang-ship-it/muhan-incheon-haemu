...
const info = await page.evaluate(() => {
  ...
  return {
    ...
    hasR: text.includes("R 다시 시작"),
    afterOk: afterText.includes("R 다시 시작"),
...
await page.screenshot({ path: "/workspace/screenshots/help-restart-after.png" });
await browser.close();

const fail = [];
if (info.map[0] !== 96 || info.map[1] !== 96) fail.push("map " + info.map);
if (info.roles.join(",") !== "haeju,mujin,dochi,wolsim") fail.push("roles " + info.roles);
if (!info.hasR) fail.push("missing R");
