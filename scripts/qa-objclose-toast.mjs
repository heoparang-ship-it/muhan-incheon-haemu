...
await page.locator("#collapseObj").click();
...
const info = await page.evaluate(() => {
  ...
  const collapsed = !!(obj && obj.classList.contains("collapsed"));
...
await page.screenshot({ path: "/workspace/screenshots/objclose-toast-after.png" });
await browser.close();

const fail = [];
if (info.map[0] !== 96 || info.map[1] !== 96) fail.push("map " + info.map);
if (info.roles.join(",") !== "haeju,mujin,dochi,wolsim") fail.push("roles " + info.roles);
if (!info.lines.includes("목표를 접었다")) fail.push("toast " + info.lines);
if (!info.collapsed) fail.push("not folded");
