...
const info = await page.evaluate(() => {
  ...
  return {
    ...
    hasF9: text.includes("F9 깔개"),
    afterOk: afterText.includes("F9 깔개"),
...
await page.screenshot({ path: "/workspace/screenshots/help-debug-after.png" });
await browser.close();

const fail = [];
if (info.map[0] !== 96 || info.map[1] !== 96) fail.push("map " + info.map);
if (info.roles.join(",") !== "haeju,mujin,dochi,wolsim") fail.push("roles " + info.roles);
if (!info.hasF9) fail.push("missing F9");
