...
await page.screenshot({ path: "/workspace/screenshots/resume-toast-after.png" });
await browser.close();

const fail = [];
if (info.map[0] !== 96 || info.map[1] !== 96) fail.push("map " + info.map);
if (info.roles.join(",") !== "haeju,mujin,dochi,wolsim") fail.push("roles " + info.roles);
if (!info.lines.includes("시간을 이었다")) fail.push("toast " + info.lines);
if (info.shown || info.menuPause) fail.push("pause still open");
