...
const info = await page.evaluate(() => {
  ...
  return {
    ...
    text,
    afterText,
    hasE: text.includes("E") && text.includes("오른쪽 클릭"),
...
await page.evaluate(() => {
  ...
  window.__HAEMU__.beginMission();
  window.__HAEMU__.state.paused = true;
});
await page.waitForTimeout(200);
await page.screenshot({ path: "/workspace/screenshots/tut-click-after.png" });
await browser.close();

const fail = [];
if (info.map[0] !== 96 || info.map[1] !== 96) fail.push("map " + info.map);
if (info.roles.join(",") !== "haeju,mujin,dochi,wolsim") fail.push("roles " + info.roles);
if (!info.hasE) fail.push("missing keys " + info.text);
if (!info.afterText.includes("E") || !info.afterText.includes("오른쪽 클릭")) fail.push("continue text");
