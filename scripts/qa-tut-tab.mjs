...
const info = await page.evaluate(() => {
  ...
  return {
    ...
    hasTab: text.includes("Tab으로 다음 대원을 고릅니다"),
    afterOk: afterText.includes("Tab으로 다음 대원을 고릅니다"),
    snapHasTut: Object.prototype.hasOwnProperty.call(snap, "tutorialDismissed"),
    phases,
  };
});

await page.screenshot({ path: "/workspace/screenshots/tut-tab-after.png" });
await browser.close();

const fail = [];
if (info.map[0] !== 96 || info.map[1] !== 96) fail.push("map " + info.map);
if (info.roles.join(",") !== "haeju,mujin,dochi,wolsim") fail.push("roles " + info.roles);
if (!info.hasTab) fail.push("missing Tab");
