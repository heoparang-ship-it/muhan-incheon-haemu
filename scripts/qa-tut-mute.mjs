...
const info = await page.evaluate(() => {
  ...
  return {
    ...
    hasM: text.includes("M으로 소리를 끕니다"),
    afterOk: afterText.includes("M으로 소리를 끕니다"),
...
await page.screenshot({ path: "/workspace/screenshots/tut-mute-after.png" });
await browser.close();

const fail = [];
if (info.map[0] !== 96 || info.map[1] !== 96) fail.push("map " + info.map);
if (info.roles.join(",") !== "haeju,mujin,dochi,wolsim") fail.push("roles " + info.roles);
if (!info.hasM) fail.push("missing M");
