...
const info = await page.evaluate(() => {
  ...
  const read = () => {
    ...
    return !!(tut && !tut.hidden && tut.offsetParent !== null);
  };
  const atStart = read();
...
await page.evaluate(() => window.__HAEMU__.setMenuPause(false));
await page.waitForTimeout(200);
await page.screenshot({ path: "/workspace/screenshots/tut-hud-after.png" });
await browser.close();

const fail = [];
if (info.map[0] !== 96 || info.map[1] !== 96) fail.push("map " + info.map);
if (info.roles.join(",") !== "haeju,mujin,dochi,wolsim") fail.push("roles " + info.roles);
if (!info.atStart) fail.push("start hidden");
if (info.afterContinue) fail.push("continue shown");
if (!info.afterNew) fail.push("new hidden");
