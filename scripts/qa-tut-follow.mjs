...
const info = await page.evaluate(() => {
  ...
  return {
    ...
    hasF: text.includes("F로 선택 대원을 따라갑니다"),
    afterOk: afterText.includes("F로 선택 대원을 따라갑니다"),
...
await page.screenshot({ path: "/workspace/screenshots/tut-follow-after.png" });
await browser.close();

const fail = [];
if (info.map[0] !== 96 || info.map[1] !== 96) fail.push("map " + info.map);
if (info.roles.join(",") !== "haeju,mujin,dochi,wolsim") fail.push("roles " + info.roles);
if (!info.hasF) fail.push("missing F");
