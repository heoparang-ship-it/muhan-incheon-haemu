...
const info = await page.evaluate(() => {
  ...
  return {
    ...
    hasClick: text.includes("오른쪽 클릭"),
    afterClick: afterText.includes("오른쪽 클릭"),
...
await page.screenshot({ path: "/workspace/screenshots/help-click-after.png" });
await browser.close();

const fail = [];
if (info.map[0] !== 96 || info.map[1] !== 96) fail.push("map " + info.map);
if (info.roles.join(",") !== "haeju,mujin,dochi,wolsim") fail.push("roles " + info.roles);
if (!info.hasClick) fail.push("missing click");
if (!info.afterClick) fail.push("continue text");
