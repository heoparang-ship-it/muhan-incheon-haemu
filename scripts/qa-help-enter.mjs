...
const info = await page.evaluate(() => {
  ...
  return {
    ...
    hasEnter: text.includes("Enter 일제 실행"),
    afterOk: afterText.includes("Enter 일제 실행"),
...
await page.screenshot({ path: "/workspace/screenshots/help-enter-after.png" });
await browser.close();

const fail = [];
if (info.map[0] !== 96 || info.map[1] !== 96) fail.push("map " + info.map);
if (info.roles.join(",") !== "haeju,mujin,dochi,wolsim") fail.push("roles " + info.roles);
if (!info.hasEnter) fail.push("missing Enter");
