...
const info = await page.evaluate(() => {
  ...
  return {
    ...
    hasArrows: text.includes("방향키"),
    afterOk: afterText.includes("방향키"),
...
await page.screenshot({ path: "/workspace/screenshots/help-arrows-after.png" });
await browser.close();

const fail = [];
if (info.map[0] !== 96 || info.map[1] !== 96) fail.push("map " + info.map);
if (info.roles.join(",") !== "haeju,mujin,dochi,wolsim") fail.push("roles " + info.roles);
if (!info.hasArrows) fail.push("missing arrows");
