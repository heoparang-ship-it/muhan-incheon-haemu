#!/usr/bin/env python3
"""Stitch tutorial close-ups into a 4× atlas and mip chain.

Layout stays the official 1065×737. Generated tiles invent crates/cars,
so we only add high-frequency detail where color still matches the map.
"""
...
REF = Path(__file__).with_name("tut-zoom4-tiles.json")

Z1 = MAPS / "tut01_ground.png"
Z0 = MAPS / "tut01_ground_z0.jpg"      # 4×
Z_MID = MAPS / "tut01_ground_z1.png"   # 2×
Z2 = MAPS / "tut01_ground_z2.png"      # ½
# keep old png name unused; game will load jpg for z0
...
OBJ_OX = {"cart": 0.88}
OBJ_SCALE = 3   # 3× sheets — enough for zoom ~3


def load_boxes() -> list[tuple[str, tuple[int, int, int, int]]]:
    tiles: list[tuple[str, tuple[int, int, int, int]]] = []
    if REF.exists():
        for t in json.loads(REF.read_text()):
            tiles.append((f"zoom4_{t['name']}.png", tuple(t["box"])))
    extras = [
...
def add_tile(canvas: np.ndarray, path: Path, box, amount: float = 1.0) -> None:
    if not path.exists():
    ...
    hp = highpass(src, sigma=2.8)
    src_a = np.asarray(src, np.float32)
    dist = np.sqrt(((src_a - base) ** 2).sum(axis=2))
    trust = np.clip(1.0 - dist / 62.0, 0.0, 1.0)[..., None]
    mask = feather(th, tw, pad=110)[..., None] * trust
    canvas[y0:y1, x0:x1] = np.clip(base + hp * amount * mask, 0, 255)
    print("tile", path.name, box, "shift", sx, sy, "trust", float(trust.mean()))
...
def write_objects() -> dict:
    ...
    for name, h in OBJ_LOGICAL.items():
        raw = Image.open(ART / f"obj_{name}.png")
        keyed = chroma_clean(raw)
        trimmed = trim_alpha(keyed)
        hi = scale_to_height(trimmed, h * OBJ_SCALE)
        hi.save(dest_dir / f"{name}.png", "PNG", optimize=True)
        out[name] = {
            "w": hi.width, "h": hi.height,
            "dw": int(round(hi.width / OBJ_SCALE)), "dh": h,
            "ox": 0.50, "oy": OBJ_OX.get(name, 0.90),
        }
        print("obj", OBJ_SCALE, "x", name, hi.size, "draw", out[name]["dw"], out[name]["dh"])
    return out
...
def main() -> None:
    ...
    for fname, box in load_boxes():
        add_tile(canvas, ART / fname, box)
    z0 = Image.fromarray(np.clip(canvas, 0, 255).astype(np.uint8), "RGB")
    z0 = z0.filter(ImageFilter.UnsharpMask(radius=1.6, percent=70, threshold=2))

    # 4× is huge as PNG — JPEG holds paint grain well enough
    z0.save(Z0, "JPEG", quality=90, optimize=True)
    z0.save(ART / "tut01_ground_z0.jpg", "JPEG", quality=90, optimize=True)
    old_png = MAPS / "tut01_ground_z0.png"
    if old_png.exists():
        old_png.unlink()

    z_mid = z0.resize((TUT_W * 2, TUT_H * 2), Image.Resampling.LANCZOS)
    z_mid.save(Z_MID, "PNG", optimize=True)
    z_mid.save(ART / "tut01_ground_z1.png", "PNG", optimize=True)

    z2 = z1.resize((TUT_W // 2, TUT_H // 2), Image.Resampling.LANCZOS)
    z2.save(Z2, "PNG", optimize=True)

    # compare: blurry 4× upscale vs stitch, 1:1 pixels
    cmp = Image.new("RGB", (720, 400))
    a = hi.crop((240, 120, 600, 320)).resize((360, 200), Image.Resampling.NEAREST)
    b = z0.crop((240, 120, 600, 320)).resize((360, 200), Image.Resampling.NEAREST)
    cmp.paste(a, (0, 0))
    cmp.paste(b, (360, 0))
    c = hi.crop((1680, 880, 2040, 1080)).resize((360, 200), Image.Resampling.NEAREST)
    d = z0.crop((1680, 880, 2040, 1080)).resize((360, 200), Image.Resampling.NEAREST)
    cmp.paste(c, (0, 200))
    cmp.paste(d, (360, 200))
    ...
    meta["mips"] = {
        "z0": {"file": "tut01_ground_z0.jpg", "w": HI_W, "h": HI_H, "scale": 4, "zoom": 2.05},
        "z1": {"file": "tut01_ground_z1.png", "w": TUT_W * 2, "h": TUT_H * 2, "scale": 2, "zoom": 1.12},
        "z2": {"file": "tut01_ground.png", "w": TUT_W, "h": TUT_H, "scale": 1, "zoom": 0.62},
        "z3": {"file": "tut01_ground_z2.png", "w": TUT_W // 2, "h": TUT_H // 2, "scale": 0.5, "zoom": 0.0},
    }
    meta["objects"] = objs
    meta_path.write_text(json.dumps(meta, ensure_ascii=False, indent=2) + "\n")
    print("z0", z0.size, Z0.stat().st_size)
    print("z1", z_mid.size, Z_MID.stat().st_size)
