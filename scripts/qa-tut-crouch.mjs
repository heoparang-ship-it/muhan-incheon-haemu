...
const info = await page.evaluate(() => {
  ...
  return {
    ...
    hasC: /웅크리기나\s*C/.test(text) || text.includes("웅크리기나 C"),
    afterOk: afterText.includes("C") && afterText.includes("웅크리기"),
...
await page.screenshot({ path: "/workspace/screenshots/tut-crouch-after.png" });
await browser.close();

const fail = [];
if (info.map[0] !== 96 || info.map[1] !== 96) fail.push("map " + info.map);
if (info.roles.join(",") !== "haeju,mujin,dochi,wolsim") fail.push("roles " + info.roles);
if (!info.hasC) fail.push("missing C");
