...
await page.locator("#tutClose").click();
...
await page.screenshot({ path: "/workspace/screenshots/tutclose-toast-after.png" });
await browser.close();

const fail = [];
if (info.map[0] !== 96 || info.map[1] !== 96) fail.push("map " + info.map);
if (info.roles.join(",") !== "haeju,mujin,dochi,wolsim") fail.push("roles " + info.roles);
if (!info.lines.includes("첫 동선을 접었다")) fail.push("toast " + info.lines);
if (!info.tutGone) fail.push("tutorial still there");
