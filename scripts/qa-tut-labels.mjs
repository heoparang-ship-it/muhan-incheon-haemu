...
const info = await page.evaluate(() => {
  ...
  return {
    ...
    hasV: text.includes("V로 이름표를 켭니다"),
    afterOk: afterText.includes("V로 이름표를 켭니다"),
...
await page.screenshot({ path: "/workspace/screenshots/tut-labels-after.png" });
await browser.close();

const fail = [];
if (info.map[0] !== 96 || info.map[1] !== 96) fail.push("map " + info.map);
if (info.roles.join(",") !== "haeju,mujin,dochi,wolsim") fail.push("roles " + info.roles);
if (!info.hasV) fail.push("missing V");
