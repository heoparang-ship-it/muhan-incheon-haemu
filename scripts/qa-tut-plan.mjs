...
const info = await page.evaluate(() => {
  ...
  return {
    ...
    hasSpace: text.includes("Space") && text.includes("계책"),
    afterOk: afterText.includes("Space") && afterText.includes("계책"),
...
await page.screenshot({ path: "/workspace/screenshots/tut-plan-after.png" });
await browser.close();

const fail = [];
if (info.map[0] !== 96 || info.map[1] !== 96) fail.push("map " + info.map);
if (info.roles.join(",") !== "haeju,mujin,dochi,wolsim") fail.push("roles " + info.roles);
if (!info.hasSpace) fail.push("missing Space");
