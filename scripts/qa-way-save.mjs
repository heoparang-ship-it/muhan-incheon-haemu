...
const info = await page.evaluate(() => {
  ...
  H.state.paused = true;
  const read = () => {
    const btn = document.getElementById("wayBtn");
    return {
      on: !!H.state.waypointMode,
      active: !!(btn && btn.classList.contains("active")),
    };
  };

  H.setWaypointMode(true, { quiet: true });
  const afterSet = read();
  ...
  return {
    ...
    snapOn: snap.waypointMode,
    afterSet,
    afterRebuild,
    afterApply,
    afterContinue,
    hasSet: typeof H.setWaypointMode === "function",
    phases,
  };
});

await page.waitForTimeout(200);
await page.screenshot({ path: "/workspace/screenshots/way-save-after.png" });

const oldInfo = await page.evaluate(() => {
  const H = window.__HAEMU__;
  const old = H.snapshotSave();
  delete old.waypointMode;
  H.setWaypointMode(true, { quiet: true });
  H.applySave(old);
  const btn = document.getElementById("wayBtn");
  return {
...
await page.screenshot({ path: "/workspace/screenshots/way-save-clear.png" });
await browser.close();
...
if (!info.hasSet) fail.push("setWaypointMode 없음");
