...
const info = await page.evaluate(() => {
  ...
  return {
    ...
    hasKeys: text.includes("1~4") && text.includes("대원"),
    afterOk: afterText.includes("1~4") && afterText.includes("대원"),
...
await page.screenshot({ path: "/workspace/screenshots/tut-agent-after.png" });
await browser.close();

const fail = [];
if (info.map[0] !== 96 || info.map[1] !== 96) fail.push("map " + info.map);
if (info.roles.join(",") !== "haeju,mujin,dochi,wolsim") fail.push("roles " + info.roles);
if (!info.hasKeys) fail.push("missing 1-4");
