...
const info = await page.evaluate(() => {
  ...
  return {
    ...
    hasWasd: text.includes("WASD"),
    hasWheel: text.includes("휠"),
    afterOk: afterText.includes("WASD") && afterText.includes("휠"),
...
await page.screenshot({ path: "/workspace/screenshots/tut-cam-after.png" });
await browser.close();

const fail = [];
if (info.map[0] !== 96 || info.map[1] !== 96) fail.push("map " + info.map);
if (info.roles.join(",") !== "haeju,mujin,dochi,wolsim") fail.push("roles " + info.roles);
if (!info.hasWasd) fail.push("wasd");
if (!info.hasWheel) fail.push("wheel");
